...
# 그 외
df_both = df[(~(df["품질등급"] == "불량")) & (df["실린더압력"].between(210, 220))]
print(len(df_both)) # 19
df_both2 = df[(df["품질등급"] == "불량") & (df["실린더압력"].between(210, 220))]
print(len(df_both2)) # 5
df_str_contains = df[ df["품질등급"].str.contains("불량")]
print(len(df_str_contains)) # 6
print(len(df[ (df["품질등급"].str.contains("불량")) & (df["품질등급"].isin(["불량"])) ]))
# 6, 둘은 같은 조건이다